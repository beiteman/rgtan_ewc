...
def rgtan_main(feat_df, graph, train_idx, test_idx, labels, args, cat_features, neigh_features: pd.DataFrame, nei_att_head):
    ...
    lambda_ewc = args.get('lambda_ewc', 0.1)  # 加在 args 裡，或給預設
    ewc = None  # 預設先沒有 EWC
    ...
    for fold, (trn_idx, val_idx) in enumerate(kfold.split(feat_df.iloc[train_idx], y_target)):
        ...
        if lambda_ewc > 0:
            ewc_obj = EWC(model, train_dataloader, device, loss_fn)
        for epoch in range(start_epoch, args['max_epochs']):
            ...
            if ewc_obj is not None:
                loss += lambda_ewc * ewc_obj.compute_ewc_loss(model)
                task_loss = loss_fn(train_batch_logits, batch_labels)
                if ewc is not None:
                    penalty = ewc.penalty(model)
                    train_loss = task_loss + lambda_ewc * penalty
                else:
        ...
        if fold == 0:
            print("🔁 Saving EWC state after fold 0")
            # ewc = EWC(model=earlystoper.best_model.to(device),
            #         dataloader=train_dataloader,
            #         loss_fn=loss_fn,
            #         device=device)
            all_train_sampler = MultiLayerFullNeighborSampler(args['n_layers'])
            all_train_dataloader = NodeDataLoader(
                graph, trn_ind, all_train_sampler,
                device=device, batch_size=args['batch_size'], shuffle=False, drop_last=False, num_workers=0
            )
            ewc = EWC(model=earlystoper.best_model.to(device),
                    dataloader=all_train_dataloader,
                    loss_fn=loss_fn,
                    device=device)
